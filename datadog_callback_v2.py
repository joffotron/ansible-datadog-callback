...
class CallbackModule(CallbackBase):
    ...
    def _load_conf(self, file_path):
        conf_dict = self._decrypt_and_load(file_path)
        return conf_dict.get('api_key', ''), conf_dict.get('url', 'https://app.datadoghq.com')
        raise 'Debugging!'

    def _decrypt_and_load(self, conf_file_path):      
        with open(conf_file_path, 'r') as conf_file:
            data = conf_file.read()

        pl = getattr(self, 'play', None)
        for i in dir(pl._variable_manager): print i
        
        vault = VaultLib(password=self._options._vault_password)
        if vault.is_encrypted(data):    
            if self._options._vault_password is None:
                raise errors.AnsibleError("A vault password must be specified to decrypt %s" % conf_file)
            data = yaml.load(vault.decrypt(data))
        else:
            data = yaml.load(data)    
    ...
    def _send_event(self, title, alert_type=None, text=None, tags=None, host=None, event_type=None, event_object=None):
        if datadog.api._api_key is None:
